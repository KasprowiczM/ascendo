#!/usr/bin/env bash
# =============================================================================
# adapters/macos/scripts/softwareupdate/cleanup.sh -- no-op OS-update cleanup phase
# =============================================================================
# softwareupdate has no local caches to prune. Emits a success sidecar with
# one info message and zero items so the orchestrator's per-(phase,category)
# accounting works uniformly across all sources.
#
# Args:
...
# -- init sidecar --------------------------------------------------------------
json_init "cleanup" "softwareupdate" "$RUN_ID" "$TRIGGER" "$PROFILE_NAME" \
...
# -- no-op body ----------------------------------------------------------------
json_add_message "info" "softwareupdate cleanup: no caches to prune. macOS manages its own update staging."
