<#
.SYNOPSIS
  Microsoft Store plan phase. Side-effect-free: emits only items that
  WOULD be touched by an apply (i.e. those with available upgrades).
#>
[CmdletBinding()]
param(
...
try {
    $_sidecarArgs = @{
        ...
        DryRun = [bool]$DryRun; Phase = 'plan'; Category = 'msstore'
    ...
    try { $_all = @(Get-WingetUpgradable -ErrorAction Stop); $upgradable = @($_all | Where-Object { $_.PSObject.Properties['Source'] -and $_.Source -and ($_.Source -ieq 'msstore') }) } catch {
        Add-SidecarMessage -Sidecar $sidecar -Level 'warn' `
            -Text ("plan enumeration failed: {0}" -f $_.Exception.Message)
    }
    ...
    Add-SidecarMessage -Sidecar $sidecar -Level 'info' `
        -Text ("msstore plan: {0} packages would be upgraded" -f $upgradable.Count)
    ...
    if ($null -ne $sidecar) {
        try {
            Add-SidecarMessage -Sidecar $sidecar -Level 'error' `
                -Text ("msstore plan infrastructure failure: {0}" -f $_.Exception.Message)
            [void](Save-Sidecar -Sidecar $sidecar -OutputDir $OutputDir)
        } catch {}
    }
    exit 1
