...
class GitHubReleaseConfig(BaseModel):
    ...
    release's ``tag_name`` as the candidate version. Apply remains Tier-B
    (trigger-only) in v1 — we surface the candidate version and emit a
    ``triggered`` item that points at the GitHub release page, rather than
    downloading + running the .exe (which would need Authenticode + UAC +
    per-installer flag handling that we'll add in a follow-up).
