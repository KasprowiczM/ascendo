...
#   7. Doctor + tag    -- `git tag -a v0.4.1`. Does NOT push.
#
# Flags:
...
if [ "$DO_MAS" -eq 1 ]; then
    ...
    if [ -z "${SUDO_PW:-}" ]; then
        ...
        note "       v0.4.1 tag exit bar. Stage 5b itself uses sudo interactively,"
...
if [ "$NO_TAG" -eq 1 ]; then
    ...
    warn "apply did not succeed cleanly (exit $APPLY_RC) -- refusing to tag v0.4.1."
    exit 1
elif [ "$VERIFY_RC" -gt 1 ]; then
    warn "verify exited $VERIFY_RC (>1) -- refusing to tag v0.4.1."
    exit 1
elif git rev-parse v0.4.1 >/dev/null 2>&1; then
    note "tag v0.4.1 already exists -- skipping."
else
    git tag -a v0.4.1 \
        -m "macOS adapter M5.7.1 — Web vendor probes + bug fixes (sparkle/github_dmg/keystone/squirrel/builtin/msupdate/docker handlers); v0.4.1 (apply RC=$APPLY_RC)"
    note "tagged v0.4.1. Run 'git push --tags' when ready."
fi
