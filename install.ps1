...
.PARAMETER Edition
    basic — Simplified UI, sensible defaults; for everyday end-users. (Default.)
...
.PARAMETER Profile
    ...
    desktop — adds Tauri 2.x desktop shell prerequisites.
...
param(
    [switch] $Reinstall,
    [switch] $Update,
    [ValidateSet('basic','dev')]                  [string] $Edition  = $env:ASCENDO_EDITION,
    [ValidateSet('cli','web','desktop','full')]   [string] $Profile  = $env:ASCENDO_PROFILE,
...
if (-not $Edition)  { $Edition  = 'basic' }
if (-not $Profile)  { $Profile  = 'web' }
if (-not $Language) { $Language = 'en' }

# Validate env-var-supplied values (ValidateSet only fires for explicit args).
if ($Edition -notin @('basic','dev')) {
    Write-Host "[fail]  Invalid edition: '$Edition'. Use basic or dev." -ForegroundColor Red
    exit 2
}
if ($Profile -notin @('cli','web','desktop','full')) {
    Write-Host "[fail]  Invalid profile: '$Profile'. Use cli, web, desktop, or full." -ForegroundColor Red
    exit 2
}
...
$WantWeb     = ($Profile -eq 'web')     -or ($Profile -eq 'full')
$WantDesktop = ($Profile -eq 'desktop') -or ($Profile -eq 'full')
...
Write-Info "Mode: $(if($Update){'update'}else{'install'})   Edition: $Edition   Profile: $Profile   Lang: $Language   NonInteractive: $NonInteractive"
...
# Edition marker (dashboard reads on startup)
$editionMarker = Join-Path $INSTALL_DIR '.ascendo-edition'
Set-Content -Path $editionMarker -Value $Edition -Encoding UTF8 -NoNewline
Add-Content -Path $editionMarker -Value "`n" -Encoding UTF8 -NoNewline
Write-OK "Edition marker: $editionMarker ($Edition)"

# User helper scripts -> %LOCALAPPDATA%\Microsoft\WindowsApps as .cmd shims.
# Every PowerShell helper (ascendo_*.ps1) gets a matching .cmd that calls
# pwsh.exe (PS 7) or powershell.exe (PS 5.1) on the underlying script. POSIX
# helpers (no extension) are skipped on Windows. dev/ subdir only loops
# when -Edition dev.
Write-Step "Helper scripts"
...
function New-AscendoShim {
    param([string] $SourcePs1)
    $name = [System.IO.Path]::GetFileNameWithoutExtension($SourcePs1)  # e.g. ascendo_start_web
    $shim = Join-Path $WINDOWSAPPS "$name.cmd"
    $body = @"
@echo off
REM Ascendo helper-script shim — generated by install.ps1.
...
if (Test-Path $userScriptsDir) {
    Get-ChildItem -Path $userScriptsDir -Filter 'ascendo_*.ps1' -File -ErrorAction SilentlyContinue | ForEach-Object {
        New-AscendoShim -SourcePs1 $_.FullName | Out-Null
        $linkedCount++
    }
    if ($Edition -eq 'dev') {
...
if ($linkedCount -gt 0) {
    Write-OK "Helper scripts: $linkedCount .cmd shim(s) in $WINDOWSAPPS"
} else {
    Write-Info "No helper scripts found in $userScriptsDir (sparse checkout?)"
}
...
Write-Step (T 'DONE')
Write-Host ""
Write-Host "  Installed: edition=$Edition profile=$Profile" -ForegroundColor Green
Write-Host ""
...
if ($WantDesktop) {
    Write-Host "  ascendo_start_desktop          # launch the Tauri desktop app" -ForegroundColor Green
}
Write-Host ""
Write-Host "  Repo:    $INSTALL_DIR"
Write-Host "  venv:    $VENV_DIR"
Write-Host "  edition: $editionMarker"
