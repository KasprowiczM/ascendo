...
| `Ms365` silent update hang | `msupdate --install` takes a very long time in the background even if no updates are available, giving the perception of a hang. | Modified `msupdate.sh` to abandon the silent background install entirely. Now returns exit code 95 with instructions for the user to open the native Microsoft AutoUpdate app to install updates manually. |
